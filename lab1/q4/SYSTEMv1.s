...
.eqv STOPWATCH		0xFF200510	# contador

.eqv LFSR		0xFF200514	# gerador de numeros aleatorios
...
# saber se estamos executando no rars ou na fpga
#.macro DE2(%salto)
...
# teclado que vamos usar sera um ps2, ele nao devolve um cod d tabela ascii, ele devolve um codigo
# a tabela abaixo faz essa conversao do codigo do teclado ps2 para tabela ascii
LabelScanCodeShift:
...
# rotinas do ecall
syscallException:     addi    sp, sp, -264              # aloca memoria na pilha e salva todos os registradores na pilha
    sw      x1,     0(sp)
    sw      x2,     4(sp)
    sw      x3,     8(sp)
    sw      x4,    12(sp)
    ...
    # ***OBS: parte comentada pois no rars nao temos o registrador k0
    ...
    # zerando pra garantir que nao ha nenhum lixo de memoria
    add     t0, zero, zero
    add     t1, zero, zero
    add     t2, zero, zero
    add     t3, zero, zero
...
loop1printInt:	div 	t4, a0, t2			# divide por 10 e t4=quociente
		rem     t3, a0, t2			# resto(t3= a0%t2)
...
loopReadCharKDMMIO:  	lw     	s2, 0(t0)   			# le o bit de flag do teclado
			andi 	s2, s2, 0x0001		        # masacara bit 0
			beq     s2, zero, loopReadCharKDMMIO    # testa se uma tecla foi pressionada
    			lw 	s2, 4(t0)			# le o ascii da tecla pressionada
...
buffermodificadoChar:    
#	andi    t3, t2, 0xFF00 	# mascara o 2o scancode
    	li tp,0x0000FF00
    	and t3, a3, tp
	
	li t5, 0XF000
	beq     t3, t5, teclasoltaChar		# eh 0xF0 no 2o scancode? tecla foi solta
	andi	t3, t2, 0xFF				# mascara 1o scancode
	li t5, 0x12
    	bne 	t3, t5, atualizaBufferChar		# nao eh o SHIFT que esta pressionado ? volta a ler 
...
teclasoltaChar:		andi t3, t2, 0x00FF		# mascara o 1o scancode
	li t5, 0x12
  	bgt	t3, t5, atualizaBufferChar		# se o scancode for > 0x80 entao nao eh imprimivel!
  	li t5, 0x12
	bne 	t3, t5, naoehshiftChar		# nao foi o shift que foi solto? entao processa
...
naoehshiftChar:	   	add     t3, s0,t3                   	# endereco na tabela de scancode da tecla com ou sem shift
    	lb      s1, 0(t3)				# le o ascii do caracter para $v0
    	beq     s1, zero, atualizaBufferChar		# se for caractere nao imprimivel volta a ler
...
# a0 = end Inicio      	 	        #
# a1 = tam Max String 		 	#
# $v0 = s2 = end do ultimo caractere	#
# $v1 = s3 = num de caracteres digitados#
# 2017/2                		#
#########################################
# muda s2, s3, a0 e $fp=s0  

readString: 	addi 	sp, sp, -4			# reserva espaco na pilha
		sw 	ra, 0(sp)			# salva $ra
		li 	s3, 0				# zera o contador de caracteres digitados
    		la      s0, LabelScanCode      	        # Endereco da tabela de scancode inicial para readChar
    		
loopreadString: beq 	a1, s3, fimreadString   	# buffer cheio fim
		mv 	t6, ra			        # salva $ra "substicao $t9 pelo t6"
		jal 	readChar			# le um caracter do teclado (retorno em $v0)
		mv 	ra, t6			        # recupera $ra
		li t5, 0x0A
		beq 	s2, t5, fimreadString	        # se for tecla ENTER fim
		sb 	s2, 0(a0)			# grava no buffer
...
fimreadString: 	sb 	zero, 0(a0)			# grava NULL no buffer
		addi 	s2, a0, -1			# Para que $v0 tenha o endereco do ultimo caractere digitado
		lw 	ra, 0(sp)			# recupera $ra
		addi 	sp, sp, 4			# libera espaco
		jr 	ra				# retorna
...
################################
#    ReadInt                   #
# $v0 = s2 = valor do inteiro  #
#                              #
################################

readInt: 	addi 	sp,sp,-4		# reserva espaco na pilha
	sw 	ra, 0(sp)			# salva $ra na pliha
	...
	mv 	t0, s2			        # copia endereco do ultimo digito
	li 	t2, 10				# dez
	li 	t3, 1				# dezenas, centenas, etc
	mv 	s2, zero			# zera o numero
...
loopReadInt: 	beq	s3, zero, fimReadInt	# Leu todos os digitos
	...
	li, t5, 0x2b
	beq 	t1, t5, ehposReadInt		# = '+'
	li, t5, 0x30
	blt 	t1, t5, naoehReadInt		# <'0'
	li t5, 0x39
	bgt 	t1, t5, naoehReadInt		# >'9'
	addi 	t1, t1, -48			# transforma ascii em numero
	mul 	t1, t1, t3			# multiplica por dezenas/centenas  t1= t1*t3
	add 	s2, s2, t1			# soma no numero
	mul 	t3 , t3, t2			# proxima dezena/centena "t3 = t3 * t2"
	addi 	t0, t0, -1			# busca o digito anterior
	addi	s3, s3, -1			# reduz o contador de digitos 
	j loopReadInt				# volta para buscar proximo digito

naoehReadInt:	j goToExit		# chama syscall 10 pra sair do programa

ehnegReadInt:	sub s2, zero,s2		# se for negativo 

ehposReadInt:					# se for positivo so retorna

fimReadInt:	lw 	ra, 0(sp)		# recupera $ra da pilha
	addi 	sp, sp, 4			# libera espaco
	jr 	ra				# fim ReadInt
...
midiOutSyncDE2:	li      t0, NoteData
    		...
    		# **obs: tirei pra fazer os testes logo
...
popEvent:     addi    sp, sp, -12
    ...
    lw      s4, 0(s0)   # $k0 substituido por s4
    la      s5, eventQueueEndPtr # $k1 substituido por s5
    lw      s5, 0(s5)
    li      s2, 0
    beq     s4, s5, popEventEnd

    # tira o evento da fila e coloca em $v1==s3
    mv    a0, s4
    # *** obs: tirando logo pra teste
    #jal     eventQueueIncrementPointer
    sw      s2, 0(s0)
    li      s2, 1
    lw      s3, 0(s4)
...
printFloat:	addi 	sp, sp, -4
		...
		li 	t0, '+'			        # define sinal '+'
		fcvt.w.s s1, f12			# recupera o numero float "s1 recebe fa0"
...
ehposprintFloat: sb 	t0, 0(s0)			# coloca sinal no buffer
		...
		# Encontra o expoente em $t0
		 fcvt.w.s t0, fa0	                # recupera o numero float  "t0 recebe fa0"
#		 andi 	t0, t0, 0x7F800000   		# mascara com 0111 1111 1000 0000 0000 0000...		 
		...
		fmv.x.s t1, fa0			        # recupera o numero float "t1 recebe fa0" 
		...
		li t5, 255
		beq 	t0, t5, ehExp255printFloat	# Expoente = 255
		...
		lui 	t0, 0x3F80
		fcvt.s.w f1, t0		        # f1 recebe o numero 1.0
		lui 	t0, 0x4120
		fcvt.s.w f10, t0		        # f10 recebe o numero 10.0
		
		flt.s 	t4, f0, f1		# $f0 < 1.0 ? t4=1 : t4=0
		mv s4,t4                        # s4 recebe t4
		li t5, 1
		beq t5, t4, menor1printFloat    # t4==1 ? menor1printFloat : proxima linha           
		fmv.s 	f2, f10		        # $f2  fator de multiplicacao = 10 "f2=10"
...
loop1printFloat: 	fdiv.s 	f4, f4, f2			# divide o numero pelo fator multiplicativo
		 	fle.s 	t5, f4, f1			# o numero(f4) eh > que 1? t5=0 : t5=1
		 	beq t5, zero,fimloop1printFloat         # t5==0 ? pc=fimloop1printFloat: pc=pc+4
...
cont2printFloat:	fmv.s 	f4, f0			       # inicia com o numero x 
		 	fcvt.s.w f3, zero		# contador come�a em 0
loop2printFloat:  	flt.s  t6, f4, f10			# resultado eh(f4) < que 10? t4=1 : t4=0
		 	fdiv.s 	f4, f4, f2			# divide o numero pelo fator multiplicativo
		 	li t5,1
		 	beq t6, t5, intprintFloat               # t6==1 ? pc=intprintFloat : pc=pc+4
		 	fadd.s 	f3, f3, f1			# incrementa o contador
		 	j 	loop2printFloat
			
		# Neste ponto tem-se no registrado s4==1 se f0<1, em f3 o expoente de 10 e f0 o modulo do numero e $s1 o sinal
...
intprintFloat:		fmul.s 	f4, f4, f2		# ajusta o numero
		  	fcvt.w.s t0, f4		        # aredonda f4 pra acima ou pra baixo e coloca em  t0
		  	fmv.s.x f5,t0                   # f5 recebe t0
		  	flt.s  t5, f5, f4               # f5<f4 ? t5=1 : t5=0
		  	bne t5,zero,endFloor1           # t5 != 0 ? pc=endFloor1 : pc=pc+4
           	        addi t0, t0,-1                  # t0= t0-1
	endFloor1:  	addi t0, t0,48		        # converte para ascii
...
loopfracprintFloat:  	beq t1, zero, fimfracprintFloat	        # fim dos digitos?
			fcvt.w.s t1,f4                          #aredonda f4 pra cima ou pra baixo e coloca em t1
			fcvt.s.w  f5,t1                         #f5 recebe t1
			flt.s t5,f5,f4                          # f5<f4 ? t5=1 : t5=0
			bne t5,zero,endFloor2                   # t5 != 0 ? pc=endFloor2 : pc=pc+4
			addi t1,t1,-1                           # t1=t1-1
			fcvt.s.w f5,t1
 	endFloor2:      	
		  	fsub.s 	f5, f4, f5			# parte fracionaria
		  	fmul.s 	f5, f5, f10			# mult x 10
		  	fcvt.w.s t0, f5			        # converte para inteiro
		  	fmv.s.x f6,t0                           # t0 recebe f5
		  	flt.s t5,f6,f5                          # f6<f5 ? t5=1 : t5=0
		  	bne t5,zero,endFloor3                   # t5 !=0 ? pc=enFloor3 : pc=pc+4
		  	addi t0,t0,-1                           # t0=t0-1	  	
       	endFloor3:	addi t0, t0, 48			        # converte para ascii
		  	sb   t0, 0(s0)			        # coloca no buffer
...
fimfracprintFloat: 	li 	t0,'E'			# carrega 'E'
			...
			addi 	s0, s0, 1		# incrementa endereco
		  	...
		  	li 	t0, '+'			 # carrega '+'
		  	beq s4, zero, expposprintFloat		# nao eh negativo? "s4==0 ? pc=expposprintFloat : pc=pc+4"
		  	li t0, '-'				# carrega '-'
...
readFloat: addi sp, sp, -4			# aloca espaco
	...
	mv 	s0, s2			        # ultimo endereco da string (antes do \0)
	mv 	s1, s3			        # numero de caracteres digitados
...
#	beq 	t1, 'e', insere0AreadFloat		# insere '0' antes
	li t5, 101
	beq 	t1, t5, insere0AreadFloat		# insere '0' antes
#	beq 	t1, 'E', insere0AreadFloat		# insere '0' antes
	li t5, 69
	beq 	t1, t5, insere0AreadFloat
#	beq 	t1, '.', insere0AreadFloat		#  insere '0' antes
	li t5, 46
	beq 	t1, t5, insere0AreadFloat
#	beq 	t1, '+', pulaPrimreadChar		# pula o primeiro caractere
	li t5, 43
	beq 	t1, t5, pulaPrimreadChar
...
insere0AreadFloat: mv t0, s0		# endereco do ultimo caractere
		   addi s0, s0, 1		# desloca o ultimo endereco para o proximo
	   	   ...
	   	   mv t6, s7		        # primeiro caractere "$t8 subtuido por t6"
...
#		beq 	t1,'e', insere0PreadFloat	# insere '0' depois
		li t5, 101
		beq 	t1, t5, insere0PreadFloat		# insere '0' antes
#		beq 	t1,'E', insere0PreadFloat	# insere '0' depois
		li t5, 69
		beq 	t1, t5, insere0PreadFloat		# insere '0' antes
...
inicioreadFloat:  fmv.s.x fa0,zero		# f0 Resultado inicialmente zero
		li 	t0, 10			# inteiro 10	
		fcvt.s.w  f9, t0		# passa para o C1 "$f10 subtituido pelo f9"
		li t0, 1			# inteiro 1
		fcvt.s.w  f1, t0		# passa para o C1 "F1 recebe t0"
...
#			beq 	t1, 'e', ehEreadFloat		# tem 'e'
			li t5, 101
			beq 	t1, t5, ehEreadFloat		# insere '0' antes
...
#			beq 	t1, '.', ehPontoreadFloat	# tem '.'
			li t5, 101
			beq 	t1, t5, ehPontoreadFloat		# insere '0' antes
...
intreadFloat:		fmv.s.x f2, zero			# zera parte inteira
			...
			mv 	t6, s7			# Primeiro Endereco  "$t8 substituido por t6"
loopintreadFloat: 	blt 	t0, t6, fimintreadFloat	# sai se o enderefo for < inicio da string
			lb 	t1, 0(t0)			# le o caracter
			li t5, 48
			blt 	t1, t5, erroreadFloat		# nao eh caractere valido para numero
			li t5, 57
			bgt 	t1, t5, erroreadFloat		# nao eh caractere valido para numero
			addi 	t1, t1, -48			# converte ascii para decimal
			fcvt.s.w  f2, t1			        # passa para o C1 "t1 recebe f2"
...
#			bgt 	t1, '9', erroreadFloat		# nao eh valido
			...
			addi 	t1, t1, -48			# converte ascii para decimal
			fcvt.s.w f2, t1 			# digito lido em float"f2 recebe t1 em precis�o simples"				
...
#			beq	t1, '-', potsinalnegreadFloat	# sinal do expoente esta escrito e eh positivo
			li t5, 41
			beq 	t1, t5, potsinalnegreadFloat		# insere '0' antes
			
#			beq 	t1, '+', potsinalposreadFloat	# sinal do expoente eh negativo
			li t5, 41
			beq 	t1, t5, potsinalposreadFloat		# insere '0' antes
...
# calcula o numero em $f2 o numero 10^exp
			...
			fmv.s 	f3, f9			# se o sinal for + $f3 eh 10 "$f10 subtituido pelo f9"
...
#		bne 	t1, '-', fimreadFloat	# nao eh '-' entao fim
		li t5, 45
		beq 	t1, t5, fimreadFloat		# insere '0' antes
...
############################################
time: 	DE2(timeDE2)
	li 	a7,30				# Chama o syscall do Rars
	ecall
	j 	fimTime				# saida
# ***obs: eu tirei o la, e coloquei li na instrucao abaixo.
...
############################################
sleep: DE2(sleepDE2)
	li 	a7, 32				# Chama o syscall do Rars
	ecall		
	j 	fimSleep			# Saida
# ***obs: eu tirei o la, e coloquei li na instrucao abaixo.
...
random: DE2(randomDE2)		
	li 	a7,41			# Chama o syscall do Rars
	ecall
	j 	fimRandom		# saida
